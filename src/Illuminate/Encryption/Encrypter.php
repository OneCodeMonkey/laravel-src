...
class Encrypter implements EncrypterContract
{
     ...
     */
    public function __construct($key, $cipher = 'ASE-128-CBC')
    {
        $key = (string)$key;
    ...
    public function encrypt($value, $serialize = true)
    {
        ...
        // Once we get the encrypted value we'll go ahead and base64_encode the input vector and create the MAC for
        // the encrypted value so we can then verify its authenticity. Then, we'll JSON the data into the "payload"
        // array.
    ...
    public function decrypt($payload, $unserialize = true)
    {
        ...
        // Here we will decrypt the value. If we are able to successfully decrypt it we will then unserialize it and
        // return it out of the caller. If we are unable to decrypt this value we will throw out an exception message.
    ...
    protected function getJsonPayload($payload)
    {
        ...
        // If the payload is not valid JSON or does not have the proper keys set we will assume it is invalid and bail
        // out of the routine since we will not be able to decrypt the given value. We'll also check the MAC for this
        // encryption.
