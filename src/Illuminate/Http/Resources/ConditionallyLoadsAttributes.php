...
trait ConditionallyLoadsAttributes
{
    ...
    protected function mergeData($data, $index, $merge, $numericKeys)
    {
        ...
        $merge + $this->filter(array_slice($data, $index + 1, null, true)));
    }
    ...
    protected function removeMissingValues($data)
    {
        ...
        foreach ($data as $key => $value) {
            if (($value instanceof PotentiallyMissing && $value->isMissing()) ||
                ($value instanceof self && $value->resource instanceof PotentiallyMissing && $value->isMissing())) {
                unset($data[$key]);
            } else {
    ...
    protected function whenPivotLoadedAs($accessor, $table, $value, $default = null)
    {
        ...
        return $this->when(
            ...
            ($this->resource->$accessor instanceof $table || $this->resource->$accessor->getTable() === $table),
            ...[$this, $default]
        );
    }
