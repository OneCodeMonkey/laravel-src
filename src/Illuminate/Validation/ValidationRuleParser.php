...
class ValidationRuleParser
{
     ...
     * @return array
     */
    protected function explodeExplicitRules($rule)
    {
        if (is_string($rule)) {
            return explode('|', $rule);
    ...
    /**
     * Prepare the given rule the Validator.
     *
     * @param mixed $rule
     * @return mixed
     */
    ...
    protected function mergeRulesForAttribute($results, $attribute, $rules)
    {
        ...
        $results[$attribute] = array_merge(isset($results[$attribute]) ?
            $this->explodeExplicitRules($results[$attribute]) : [], $merge);
    ...
    protected static function parseStringRule($rules)
    {
        ...
        // The format for specifying validation rules and parameters follows an easy {rule}:{parameters}
        // formatting convention. For instance the rule "Max:3" states that the value may only be three letters.
