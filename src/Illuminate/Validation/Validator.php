...
class Validator implements ValidatorContract
{
    ...
    /**
     * The array of the fallback error messages.
     *
     * @var array
     */
    ...
    protected $implicitRules = [
        'Required', 'Filled', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequierdWithoutAll',
    ...
    protected $dependentRules = [
        'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll', 'RequiredIf', 'RequiredUnless',
        'Confirmed', 'Same', 'Different', 'Unique', 'Before', 'After', 'BeforeOrEqual', 'AfterOrEqual', 'Gt', 'Lt',
        'Gte', 'Lte', 'ExcludeIf', 'ExcludeUnless',
    ];
    ...
    public function parseData(array $data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            // If the data key contains a dot, we will replace it with another character sequence so it doesn't
            // interface with dot processing when working with array based validation rules plus Arr::dot later
            // in the validations.
            if (Str::contains($key, '.')) {
    ...
    public function passes()
    {
        ...
        // We'll spin through each rule, validating the attributes attached to that rule. Any error
        // messages will be added to the containers with each of the other error messages, returning
        // true if we don't have messages.
        ...
        // Here we will spin through all of the "after" hooks on this validator and fire them off. This
        // gives the callbacks a chance to perform all kinds of other validation that needs to
        // get wrapped up in this operation.
     ...
     * @return void
     */
    protected function validateAttributes($attribute, $rule)
    {
        ...
        // an array. Then we determine if the given rule accepts other field names as parameters. If
        // so, we will replace any asterisks found in the parameters with the correct keys.
        ...
        // If the attribute is a file, we will verify that the file upload was actually successful and if
        // it wasn't we will add a failure for the attribute. Files may not successfully upload
        // if they are too large based on PHP's settings so we wil bail in this case.
    ...
    /**
     * Determine if the filed is present, or the rule implies required.
     *
     * @param object|string $rule
     * @param string $attribute
     * @param mixed $value
    ...
    protected function shouldStopValidating($attribute)
    {
        ...
        // In case the attribute has any rule that indicates that the field is required and that
        // rule already failed then we should stop validation at this point as now there is no
        // point in calling other rules with this field empty.
    ...
    public function valid()
    {
        ...
        return array_diff_key($this->data, $this->attributesThatHavaMessages());
    }
    ...
    {
        // The primary purpose of this parse is to expand any "*" rules to the all of the explicit rules
        // needed for the given data. For example the rule  names.*  would get expanded to  names.0,
        // names.1, etc. for this data.
