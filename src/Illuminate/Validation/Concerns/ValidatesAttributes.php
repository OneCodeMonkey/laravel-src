...
trait ValidatesAttributes
{
    ...
    {
        $this->requrieParameterCount(1, $parameters, 'before_or_equal');
    ...
    {
        return Carbon::hasTestNow() && is_string($value) && ($value === 'now' || Carbon::hasRelativeKeywords($value));
    }
    ...
    /**
     * Validate that an attribute contains only alpha-numeric characters, dashes(破折号), and underscores.
     *
     * @param string $attribute
     * @param mixed $value
     * @return bool
     ...
     * @return bool
     */
    public function validateAlpahNum($attribute, $value)
    {
        if (!is_string($value) && !is_numeric($value)) {
            return false;
        }
    ...
    {
        if ($this->isValidFileInstance($value) && is_array($value->getMimeType(), ['image/svg+xml', 'image/svg'])) {
            return true;
        }
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = collect($parameters)->unique()->map(function ($validation) {
            if ($validation === 'rfc') {
    ...
    public function validateExists($attribute, $value, $parameters)
    {
        ...
        // If this parameter is not specified we will guess that the columns being "verified" shares
        // the given attribute's name.
    ...
    protected function getExistCount($connection, $table, $column, $value, $parameters)
    {
        ...
        return is_array($value) ? $verifier->getMultiCount($table, $column, $value, $extra) :
    ...
    public function validateUnique($attribute, $value, $parameters)
    {
        ...
        // The second parameter position holds the name of the column that needs to be verified as
        // unique. If this parameter isn't specified we will just assume that this column to be
        // verified shares the attribute's name.
        $column = $this->getQueryColumn($parameters, $attribute);
        ...
        // The presence verified its responsible for counting rows within this store mechanism which
        // might be a relational database or any other permanent data store like Redis, etc. We will
        // use it to determine uniqueness.
        $verifier = $this->getPresenceVerifierFor($connection);
    ...
    /**
     * Prepare the given ID for query.
     *
     * @param mixed $id
     * @return int
     */
    protected function prepareUniqueId($id)
    {
        ...
        if (filter_var(FILTER_VALIDATE_INT) !== false) {
            $id = (int)$id;
        }
    ...
    {
        return isset($parameters[1]) && $parameters[1] !== 'NULL' ? $parameters[1] :
    ...
    public function validateMax($attribute, $value, $parameters)
    {
        ...
        if ($value instanceof UploadedFile && !$this->isValid()) {
            return false;
        }
    ...
    public function validateMimetypes($attribute, $value, $parameters)
    {
        ...
        return $value->getPath() !== '' && (in_array($value->getMimeTypes(), $parameters) || in_array(explode('/', $value->getMimeType())[0].'/*', $parameters));
    }
    ...
    public function validateRequired($attribute, $value)
    {
        if (is_null($value)) {
            ...
            return false;
        } elseif ((is_string($value) || $value instanceof Countable) && count($value) < 1) {
            return false;
    ...
    protected function getSize($attribute, $value)
    {
        ...
        // This method will determine if the attribute is a number, string, or file and return the
        // proper size accordingly. If it is a number, then number itself is the size. If it is a file,
        // we take kilobytes, and for a string the entire length of the string will be considered the
        // attribute size.
    ...
    public function requireParameterCount($count, $parameters, $rule)
    {
        if (count($parameters) < $count) {
            throw new InvalidArgumentException("Valiation rule $rule requires at least $count parameters.");
        }
    }
