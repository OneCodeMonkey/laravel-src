...
class Gate implements GateContract
{
    ...
    [], array $beforeCallbacks = [], array $afterCallbacks = [], callable $guessPolicyNamesUsingCallback = null)
    {
    ...
    public function raw($ability, $arguments = [])
    {
        ...
        // First we will call the "before" callbacks for the Gate. If any of these give back a non-null response,
        // we will immediately return that result in order to let the developers override all checks for some
        // authorization cases.
        ...
        // After calling the authorization callback, we will call the "after" callbacks that are registered with the
        // Gate, which allows a developer to do logging if that is required for this application. Then we'll return
        // the result.
    ...
    {
        return ($parameter->getClass() && $parameter->allowsNull()) || ($parameter->isDefaultValueAvailable() &&
    ...
    {
        if (isset($arguments[0]) && !is_null($policy = $this->getPolicyFor($arguments[0])) && $callback =
        $this->resolvePolicyCallback($user, $ability, $arguments, $policy)) {
            return $callback;
        }
    ...
    protected function resolvePolicyCallback($user, $ability, array $arguments, $policy)
    {
        ...
        return function () use ($user, $ability, $arguments, $policy) {
            // This callback will be responsible for calling the policy's before method and running this policy method
            // if necessary. This is used to when objects are mapped to policy objects in the user's configurations
            // or on this class.
            ...
            // When we receive a non-null result from this before method, we will return it as the "final" results.
            // This will allow developers to override the checks in this policy to return the result for all rules
    ...
    {
        // If this first argument is a string, that means they are passing a class name to the policy. We will remove
        // the first argument from this argument array because this policy already knows what type of models it can
        // authorize.
