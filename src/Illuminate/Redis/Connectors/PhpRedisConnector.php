...
class PhpRedisConnector implements Connector
{
    ...
    public function connectToCluster(array $config, array $clusterOptions, array $options)
    {
        ...
        return new PhpRedisClusterConnection(
            $this->createRedisClusterInterface(
        ...
        return tap(new Redis, function ($client) use ($config) {
            if ($client instanceof RedisFacade) {
                throw new LogicException(
                    extension_loaded('redis') ? 'Please remove or rename the Redis facade alias in your "app" 
                    configuration file in order to avoid collision with the PHP Redis extension.' : 'Please make sure 
                    the PHP Redis extension is installed and enabled.'
                );
            }
