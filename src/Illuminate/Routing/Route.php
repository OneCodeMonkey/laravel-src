...
use Illuminate\Routing\Matching\SchemaValidator;
...
class Route
{
    ...
    /**
     * Bind the route into a given request for execution.
     *
     * @param \Illuminate\Http\Request $request
     * @return $this
     */
    ...
    /**
     * Get the key/vakue list of parameters without null values.
     *
     * @return array
     */
    ...
    /**
     * Determine if the route only responds to HTTP request.
     *
     * @return bool
     */
    ...
    {
        if (!is_null($this->computerMiddleware)) {
            return $this->computerMiddleware;
        }

        $this->computerMiddleware = [];

        return $this->computerMiddleware = array_unique(array_merge(
            $this->middleware(),
    ...
    public static function getValidators()
    {
        ...
        // To match the route, we will use a chain of responsibility pattern with the validator implementations.
        // We will spin through each one making sure it passes and then we will know if the route as a whole matches
        // request.
        return static::$validators = [
            new UriValidator,
            new MethodValidator,
