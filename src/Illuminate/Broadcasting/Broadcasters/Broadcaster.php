...
{
    /**
     * The registered channel authentications.
     *
     * @var array
     */
    protected $channels = [];
    ...
    protected function resolveBinding($key, $value, $callbackParameters)
    {
        ...
        return $newValue === $value ? $this->resolveImplicitBindingIfPossible($key, $value, $callbackParameters) :
            $newValue;
    }

    /**
     * Resolve an explicit paramater binding if applicable.
     *
     * @param string $key
     * @param mixed $value
     * @return mixed
    ...
    protected function normalizeChannelHandlerToCallable($callback)
    {
        return is_callable($callback) ? $callback : function (...$args) use ($callback) {
            return Container::getInstance()->make($callback)->join(...$args);
        };
    }
