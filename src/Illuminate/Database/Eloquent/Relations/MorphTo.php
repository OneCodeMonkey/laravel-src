...
class MorphTo extends BelongsTo
{
    ...
    protected function getResultsByType($type)
    {
        ...
        $query = $this->replayMacros($instance->newQuery())
            ...
            ->with(array_merge($this->getQuery()->getEagerLoads(),
                (array)($this->morphableEagerLoads[get_class($instance)] ?? [])));

        $whereIn = $this->whereInMethod($instance, $ownerKey);

        return $query->{$whereIn}($instance->getTable().'.'.$ownerKey,
            $this->gatherKeysByType($type))->get();
    }
    ...
    /**
     * Specify with relations to load for a given morph type.
     *
     * @param array $with
    ...
    public function __call($method, $parameters)
    {
        ...
        // If we tried to call a method that does not exist on the parent Builder instance, we'll assume that
        // we want to call a query macro (e.g. withTrashed) that only exists on related models. We will just store
        // the call and replay it later.
