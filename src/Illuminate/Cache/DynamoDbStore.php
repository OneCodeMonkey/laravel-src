...
class DynamoDbStore implements LockProvider, Store
{
     ...
     * @return void
     */
    public function __construct(DynamoDbClient $dynamo, $table, $keyAttribute = 'key', $valueAttribute = 'value',
                                $expirationAttribute = 'expires_at', $prefix = '')
    {
        $this->dynamo = $dynamo;
        $this->table = $table;
    ...
    public function get($key)
    {
        ...
        if (isset($response['Item'][$this->valueAttribute])) {
            return $this->unserialize(
                $response['Item'][$this->valueAttribute]['S'] ?? $response['Item'][$this->valueAttribute]['N'] ?? null
            );
        }
    }
        ...
        return array_merge(collect(array_flip($keys))->map(function () {
            ...
            if ($this->isExpired($response, $now)) {
                ...
                $value = $this->unserialize(
                    $response[$this->valueAttribute]['S'] ?? $response[$this->valueAttribute]['N'] ?? null
                );
            }
    ...
    public function add($key, $value, $seconds)
    {
        try {
            $this->dynamo->putItem([
                ...
                'ConditionException' => 'attribute_not_exists(#key) OR #expires_at < :now',
                ...
                'ExpressionAttributeValues' => [
                    ':now' => [
                        'N' => (object)Carbon::now()->getTimestamp(),
                    ],
                ],
            ]);
    ...
    /**
     * Get the UNIX timestamp fpr the given number of seconds.
     *
     * @param int $seconds
     * @return int
     */
